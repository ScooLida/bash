...
eigenValues <- read_delim("pca2.eigenval", delim = " ", col_names = F)
eigenVectors <- read_delim("pca2.eigenvec", delim = "\t", col_names = T)
...
ggplot(data = eigenVectors) +
  geom_point(mapping = aes(PC1, PC2), show.legend = TRUE ) +
